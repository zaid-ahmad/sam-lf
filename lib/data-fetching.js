...
export async function getSuperAdminData(session, branch) {
    ...
    const { startOfDayUTC, endOfDayUTC } = getStartEndDateWithOffset(
        user.branchCode
    );
    ...
    const result = await prisma.lead.findMany({
        where: {
            ...
            createdAt: {
                gte: startOfDayUTC,
                lt: new Date(endOfDayUTC.getTime() + 1),
            },
        },
        orderBy: {
            createdAt: "desc",
...
export async function adminDashboardData(branch, date) {
    const { currentDateString } = getStartEndDateWithOffset(branch, date);
...
export async function superAdminDashboardData(branch) {
    const { startOfDayUTC, endOfDayUTC } = getStartEndDateWithOffset(branch);
