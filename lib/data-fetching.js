...
import { formatTimeto12Hour, getStartEndDateWithOffset } from "./utils";
...
export async function adminDashboardData(branch, date) {
    ...
    // Fetch slot templates for the branch
    ...
    for (const slot of slotTemplates) {
        ...
        leadsPerTimeSlot[slot.timeSlot] = {
            count: count,
            limit: slot.limit,
        };
    }
...
export async function superAdminDashboardData(branch, date) {
    ...
    // Fetch slot templates for the branch
    const superAdminSlotTemplates = await prisma.slotTemplate.findMany({
        where: {
            branchCode: branch,
        },
        orderBy: {
    ...
    for (const slot of superAdminSlotTemplates) {
        const count = await prisma.lead.count({
            where: {
                branch: branch,
                appointmentDateTime: {
        ...
        superAdminLeadsPerTimeSlot[slot.timeSlot] = {
            count: count,
            limit: slot.limit,
        };
    }
